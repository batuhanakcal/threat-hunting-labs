...
**Author:** Batuhan Akcal · Cyber Security Engineer (WAF & detection engineering, financial services)
...
Started from a single command in `bash_history` — an archive being pushed to S3. Pivoting on the
...
Inside that window, someone outside the organisation wrote a file into the bucket named

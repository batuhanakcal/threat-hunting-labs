...
start with no target at all. You characterise what "normal" looks like in a dataset, then hunt
...
And only **3 queries** — the *least frequency of occurrence* principle. Noise repeats; attacks
are rare.
...
Pivoting to `PCERF-L` surfaced Symantec endpoint data: 419 behaviour events, **18 marked
"Blocked"**, including:
...
a reminder that Splunk's `host` field sometimes identifies the relay, not the origin.
...
search — legitimate behaviour on a legitimate e-commerce site. Seconds later the browser requests
...
| 4 | Symantec's script-blocking policy is blocking Splunk's own collectors (1,543 events) — degrades logging integrity | **Medium** |
...
```
A GET should download, not upload. This is behavioural — it doesn't depend on knowing the domain.
...
- **Baseline hunting finds what you weren't looking for.** No IOC list, no intel feed, no alert —
  ...
  one 3-query anomaly, which unravelled into a full compromise.
- **Rarity is a signal.** The malicious domain appeared 3 times among 176,831 queries. Noise
  repeats; intrusions are quiet. Sorting *ascending* by count is as valuable as sorting descending.
...
- **Eliminating a false positive is progress.** Hours could have been lost on Symantec's "Blocked"
  events. Hashing the caller resolved it in one query — and the remaining leads got stronger.
